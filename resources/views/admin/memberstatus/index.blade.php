<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Справочник - статусы участника') }}
        </h2>
    </x-slot>
    <div class="py-4">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-4">
                <div class="mt-3">
                    <Link modal href="{{route('memberstatus.create')}}"
                          class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Добавить статус участника
                    </Link>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <x-splade-table :for="$MemberStatus">
                            <x-splade-cell edit class="w-4 hover:bg-indigo-700">
                                <Link modal href="{{ route('memberstatus.edit', $item->id) }}"
                            ...
                            <x-splade-cell delete class="w-4 flex items-center">
                                <Link
                                    ...
                                    href="{{ route('memberstatus.destroy', $item->id) }}" method="DELETE"
