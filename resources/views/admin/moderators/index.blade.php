<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Модераторы и спикеры мероприятия') }}
        </h2>
    </x-slot>
            ...
            <div class="p-2 bg-white border-b border-gray-200 shadow-sm rounded-lg">
                <x-splade-table :for="$Moderators">
                    ...
                    <x-splade-cell edit class="w-4 hover:bg-indigo-700">
                        <Link
                            href="{{ route('moderator.edit', ['event' => $item->events_id, 'moderator' => $item->id]) }}">
                        Edit
                        </Link>
                    </x-splade-cell>
                    <x-splade-cell delete class="w-4 bg-gray-200 justify-self-start">
                        <div class="justify-self-start">
                            <Link
                                ...
                                href="{{ route('moderator.destroy', ['event' => $item->events_id, 'moderator' => $item->id]) }}"
                                method="DELETE">
                            Delete
                            </Link>
                        </div>
